                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="filter_select_status" id="filter_select_status" class="form-control" onchange="search_with_filter()">
                                    ...
                                    <option value="1" @if(!empty($params['status']) && $params['status'] == 1) selected @endif>Need Review</option>
                                    <option value="2" @if(!empty($params['status']) && $params['status'] == 2) selected @endif>Correction</option>
                                    <option value="3" @if(!empty($params['status']) && $params['status'] == 3) selected @endif>Approved</option>
                                    <option value="4" @if(!empty($params['status']) && $params['status'] == 4) selected @endif>Rejected</option>
                                </select>
                            </div>
                        </div>
                        <br>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div style="container">
                        ...
                        @foreach($applications->groupBy('booking_id') as $key =>  $booking)
                        {{-- @foreach ($applications as $key => $application) --}}
                        {{-- {{dd($booking[0],$key)}} --}}
                            <div class="row a_project_application">
                                <div class="col-md-3"><a href="{{route('application.show',['application'=>$booking[0]->id])}}">{{ $booking[0]->booking->title }}</a></div>
                                <div class="col-md-2">{{ $booking[0]->start }}</div>
                                <div class="col-md-2">{{ $booking[0]->created_at }}</div>
                                <div class="col-md-1">{{ $booking[0]->booking->user->name }}</div>
                                <div class="col-md-2">@if(empty($booking[0]->booking->user->contact)){{ $booking[0]->booking->user->email }}@else {{ $booking[0]->booking->user->contact }} @endif</div>
                                <div class="col-md-2">
                                    @switch($booking[0]->status)
                                        @case(1)
                                            ...
                                            {!! Form::open(['route' => ['application.update',$booking[0]->id], 'method' => 'put', 'style'=>'display: none;', 'id' => 'approve-form']) !!}
                                                <input type="hidden" name="status" value="2">
                                                @csrf
                                            </form>
                                            {!! Form::open(['route' => ['application.update',$booking[0]->id], 'method' => 'put', 'style'=>'display: none;', 'id' => 'reject-form']) !!}
                                                <input type="hidden" name="status" value="4">
                                                @csrf
                                            </form>
                                            {!! Form::open(['route' => ['application.update',$booking[0]->id], 'method' => 'put', 'style'=>'display: none;', 'id' => 'correction-form']) !!}
